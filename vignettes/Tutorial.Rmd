...
The app reads RBPC's Excel file. Because Excel files are inherently non-standard, their content must be strongly standardized to be properly interpreted by the application.
...
This tab also enable to visualize the DTM to check for potential issues.

## Panel 4: Tree Layout

![](page4.png){width=100%}

From the Excel database, the app produces a map of the blocks and a map of the trees. This pages shows the theoretical position of every single tree. To build an accurate map, the routines need to know the block sizes (default 18.6x18.6 m), the number of trees per block (default 6x6), and the ordering of the trees within each block. This is controlled by the input fields provided on this page. All this information is expected to be known by the user and is usually recorded somewhere in the database, often in a format that is not computer-readable thus requiring operator intervention to set up the configuration.
